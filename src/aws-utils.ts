...
export async function getEcrRegistry(): Promise<string> {
	const response: DescribeRepositoriesCommandOutput = await ecrClient.send(new DescribeRepositoriesCommand({}));

	const repositoryUri: string | undefined = response?.repositories?.[0]?.repositoryUri?.split("/")[0];
...
export async function checkIfEcrImageExists(repository: string, imageTag: string): Promise<void> {
	try {
